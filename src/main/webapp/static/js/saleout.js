$(function(){
	...
	$('#saleDate').datebox('calendar').calendar({
	...
	// 设置单号
	$('#saleNumber').html(genOddNumbers('XS'));
	...
	$('#dg2').datagrid({
		onDblClickRow:function(rowIndex,rowData){
			...
			$('#sellingPrice').val('￥'+rowData.sellingPrice);
			$('#price').val(rowData.sellingPrice);
			$('#num').focus();
		}
	});
 ...
 * 打开销售入库商品选择窗口
 */
function openSaleListGoodsDialog(){
	$('#dlg').dialog({
		title:'销售入库商品选择',
		closed:false,
		top:$(window).height()/6,
...
function openGoodsChooseDialog(){
	...
	$('#sellingPrice').val('￥'+selections[0].sellingPrice);
	$('#price').val(selections[0].sellingPrice);
	$('#num').focus();
}
	...
	if(isExist){
		$('#dg').datagrid('updateRow',{
			...
			row:{
				...
				sellingPrice:selections[0].sellingPrice,
				price:$('#price').val(),
...
function saveSaleData(){
	...
	$('#fm').form('submit',{
		...
		onSubmit:function(){
			...
			if($('#customer').combobox('getData')===null
				|| $('#customer').combobox('getData')===''
				|| $('#customer').combobox('getValue')===null
				|| $('#customer').combobox('getValue')===''){
				$.messager.alert({
					title:'系统提示',
					msg:'请选择客户',
					icon:'error', 
